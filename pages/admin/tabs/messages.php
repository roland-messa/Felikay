...
$newsletters = $pdo->query("SELECT * FROM newsletter_felykay ORDER BY created_at DESC")->fetchAll();
?>
...
<div class="grid grid-cols-1 gap-8">
      ...
      <table class="w-full text-left text-xs">
        <thead>
          <tr class="bg-slate-50 text-slate-500 uppercase font-bold border-b border-slate-100">
            ...
            <th class="p-4">Nom complet du Client</th>
            <th class="p-4">Sujet</th>
            <th class="p-4">Numero de telephone</th>
            <th class="p-4">Adresse mail</th>
        ...
        <tbody class="divide-y divide-slate-100">
          <?php if (empty($messages)): ?>
            <tr>
              <td colspan="7" class="p-10 text-center text-slate-400 italic">Aucun message pour le moment.</td>
            </tr>
          <?php endif; ?>
            ...
            <tr class="hover:bg-slate-50/50 transition">
              ...
              <td class="p-4">
                <?php if (!empty($m['email'])): ?>
                  <a href="mailto:<?= $m['email'] ?>" class="text-blue-500 hover:underline">
  ...
  <div class="bg-white rounded-3xl shadow-sm border border-slate-200 p-8">
    <div class="flex justify-between items-center mb-8">
      <div>
        <h3 class="text-sm font-bold uppercase tracking-widest text-slate-800">Inscriptions Club Felikay</h3>
        <p class="text-[10px] text-slate-400 mt-1">Liste des clients inscrits via téléphone</p>
      </div>
    ...
    <div class="flex flex-wrap gap-3">
      ...
      <?php foreach ($newsletters as $n): ?>
        <div class="group flex items-center gap-3 px-4 py-3 bg-slate-50 hover:bg-white rounded-2xl border border-slate-100 hover:border-black hover:shadow-md transition-all duration-300">
          ...
          <div>
            <span class="text-[11px] font-bold text-slate-800 block"><?= htmlspecialchars($n['nom_complet']) ?></span>

            <span class="text-[10px] text-blue-600 font-medium block italic"><?= htmlspecialchars($n['telephone']) ?></span>

            <span class="text-[9px] text-slate-400 uppercase font-medium">
              Inscrit le <?= date('d/m/Y', strtotime($n['created_at'])) ?>
            </span>
          </div>
        </div>
      <?php endforeach; ?>
