...
try {
  ...
  if ($genre_filter !== 'tous') {
    ...
    $sql .= " AND (genre = :genre OR genre = 'unisex' OR genre = 'tous')";
  }

  $sql .= " ORDER BY created_at DESC";

  $stmt = $pdo->prepare($sql);
  if ($age_filter !== 'tous') $stmt->bindValue(':age', $age_filter);
  if ($genre_filter !== 'tous') $stmt->bindValue(':genre', $genre_filter);
...
if (basename($_SERVER['PHP_SELF']) == 'ArticleEnfants.php') :
  ...
  if ($age_filter === '0-12m') $displayTitle = "Univers Nouveau-né";
  if ($age_filter === '12-18a') $displayTitle = "Collection Junior";
?>
    ...
    <section class="max-w-[1400px] mx-auto px-6">
      ...
      <div class="flex flex-col items-center mb-16">
        ...
        <div class="flex gap-8 text-[11px] uppercase tracking-[0.3em] text-stone-400">
          <a href="?age=<?php echo $age_filter; ?>&genre=tous" class="filter-link <?php echo ($genre_filter === 'tous') ? 'active' : ''; ?> pb-2 transition">Tous</a>
          <a href="?age=<?php echo $age_filter; ?>&genre=garcon" class="filter-link <?php echo ($genre_filter === 'garcon') ? 'active' : ''; ?> pb-2 transition">Garçons</a>
          <a href="?age=<?php echo $age_filter; ?>&genre=fille" class="filter-link <?php echo ($genre_filter === 'fille') ? 'active' : ''; ?> pb-2 transition">Filles</a>
        </div>
      </div>
      ...
      <div class="swiper childCarousel pb-20">
        <div class="swiper-wrapper">
          <?php foreach ($articles_enfants as $item) :
            // Nettoyage du chemin d'image pour éviter les erreurs d'affichage
          ...
          ?>
            <div class="swiper-slide group">
              ...
              <div class="text-left px-2">
                <h3 class="text-[10px] uppercase tracking-widest font-bold text-gray-900 mb-1"><?php echo $item['nom']; ?></h3>
      ...
      <?php if (empty($articles_enfants)): ?>
        <div class="text-center py-20">
          <p class="text-stone-400 italic">Désolé, aucun article ne correspond à cette sélection pour le moment.</p>
